...
- To specify the [map projection](https://en.wikipedia.org/wiki/Map_projection) used for recording the crime locations, click on the **Select CRS** button. You will find this is directly next to the `invalid projection` message. In the **Filter** box type: `EPSG 4326`. Only one option should show up: `WGS 84`. Select this option and click **OK**.
...
In this exercise, we have downloaded data, created a spatial data object by loading it into QGIS, and made selections to our data by clipping out our boroughs and filtering by crime type. We have also explored some styling options. The final step is to refine the styling and export the map from QGIS, which we will accomplish using the **Print Layout**:
